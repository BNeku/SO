#include "sched.h"
/*Practica 3 - Parte Extra
 * Nerea Jiménez y Pablo Miranda
 * 2ºE
 * */
...
/* Structure to store multiRR thread-specific fields */
struct multiRR_data {
...
static task_t* pick_next_task_multiRR(runqueue_t* rq)
{
	...
	/* CumultiRRent is not on the rq -> let's remove it */
	if (t)
...
static void enqueue_task_multiRR(task_t* t,runqueue_t* rq, int preempted)
{
	...
	else if(cs_data->quantumExhausted == 0 && cs_data->current_slice < cs_data->maxSlice)
		cs_data->current_slice++;
...
static void task_tick_multiRR(runqueue_t* rq)
{
	...
	if(cs_data->remaining_ticks_slice == 0 && current->runnable_ticks_left > 1){
		cs_data->quantumExhausted = 1;
	}
