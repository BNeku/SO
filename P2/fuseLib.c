...
static int my_unlink(const char *path) {
	...
	//idxNode = nodo-i del fichero
	 ...
	 //	Truncar el fichero utilizando resizeNode, lo cambiamos a tamaño 0
	 if(resizeNode(myFileSystem.directory.files[idxNode].nodeIdx, 0) < 0)
	        return -EIO;
	 //actualizamos manualmente los datos del nodo y el sistema
	 nodo = myFileSystem.directory.files[idxNode].nodeIdx;
	 myFileSystem.directory.files[idxNode].freeFile = true;
	 myFileSystem.directory.numFiles = myFileSystem.directory.numFiles - 1;
	 myFileSystem.nodes[nodo]->freeNode = true;
	 myFileSystem.nodes[nodo]->numBlocks = 0;
	 myFileSystem.nodes[nodo]->fileSize = 0;
...
static int my_read(const char *path, char *buf, size_t size, off_t offset, struct fuse_file_info *fi){
	...
	while (totalRead < bytes2Read) {
		...
		currentBlock = node->blocks[offset / BLOCK_SIZE_BYTES];
		offBlock = offset % BLOCK_SIZE_BYTES;
		//comprobamos que no sale del fichero
		if(readBlock(&myFileSystem, currentBlock, &buffer) == -1 ) {
		           fprintf(stderr,"Error reading blocks in my_write\n");
		           return -EIO;
		  }
